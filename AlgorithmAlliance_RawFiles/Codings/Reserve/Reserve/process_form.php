    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["gcash-screenshot"]["tmp_name"], $target_file)) {
            ...
            if (rename($gcash_screenshot_path, $new_file_path)) {
                ...
                $stmt = $conn->prepare("INSERT INTO reservations (first_name, middle_name, last_name, gender, address, email_address, contact_number, booking_date, booking_time, amount_paid, gcash_reference, gcash_screenshot) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("sssssssssdss", $first_name, $middle_name, $last_name, $gender, $address, $email_address, $contact_number, $booking_date, $booking_time, $amount_paid, $gcash_reference, $gcash_screenshot_path);
